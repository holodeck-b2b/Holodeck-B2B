...
public class AddReceiptSignals extends BaseHandler {
    ...
    protected InvocationResponse doProcessing(MessageContext mc) throws DatabaseException {
        ...
        Collection<EntityProxy<Receipt>> rcptsToAdd = getBundableReceipts(primaryMU);        
        ...
        for(EntityProxy<Receipt> r : rcptsToAdd) {
     ...
     */
    private Collection<EntityProxy<Receipt>> getBundableReceipts(EntityProxy<MessageUnit> primaryMU) {
        ArrayList<EntityProxy<Receipt>> rcpts = new ArrayList<EntityProxy<Receipt>>();
        ...
        try {
            ...
            if (!Utils.isNullOrEmpty(createdRcpts))
                rcpts.addAll(createdRcpts);
            Collection<EntityProxy<Receipt>> failedRcpts = 
                                    MessageUnitDAO.getMessageUnitsForPModesInState(Receipt.class, 
                                                                                   pmodes, 
        ...
        if (rcpts.size() > 1) {
            ...
            for(EntityProxy<Receipt> r : rcpts)
