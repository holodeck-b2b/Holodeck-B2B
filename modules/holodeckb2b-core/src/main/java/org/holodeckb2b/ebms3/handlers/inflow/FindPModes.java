...
public class FindPModes extends BaseHandler {
    ...
    protected InvocationResponse doProcessing(final MessageContext mc) throws AxisFault {
        ...
        try {
            ...
            final EntityProxy<UserMessage> usrMsg = (EntityProxy<UserMessage>) mc.getProperty(MessageContextProperties.IN_USER_MESSAGE);
            if (usrMsg != null) {
                log.debug("Message contains a User message unit, find the P-Mode");
                final IPMode pmode = PModeFinder.forReceivedUserMessage(usrMsg.entity);
                if (pmode == null) {
            ...
            final ArrayList<EntityProxy<ErrorMessage>>  errorSignals = (ArrayList<EntityProxy<ErrorMessage>>)
            ...
            if (!Utils.isNullOrEmpty(errorSignals)) {
                ...
                for (final EntityProxy<ErrorMessage> e : errorSignals) {
                    final IPMode pmode = findForReceivedErrorSignal(e.entity, mc);
                    if (pmode == null) {
                        // No matching P-Mode could be found for this message, return error
            ...
            final ArrayList<EntityProxy<Receipt>>  rcptSignals = (ArrayList<EntityProxy<Receipt>>)
            ...
            if (!Utils.isNullOrEmpty(rcptSignals)) {
                ...
                for (final EntityProxy<Receipt> r : rcptSignals) {
                    final IPMode pmode = getPModeFromRefdMessage(r.entity.getRefToMessageId());
                    if (pmode == null) {
                        // No matching P-Mode could be found for this message, return error
     ...
     */
    private IPMode findForReceivedErrorSignal(final ErrorMessage e, final MessageContext mc) {
        final IPMode pmode = null;
     ...
     */
    protected static IPMode getPModeFromRefdMessage(final String refToMsgId) {
        IPMode pmode = null;
