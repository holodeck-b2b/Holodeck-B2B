...
public class PartyInfo {
    ...
    private static class TradingPartner {
         ...
         */
        public static OMElement createElement(final ElementName rootName, final OMElement piElement, final ITradingPartner data) {
            final OMFactory f = piElement.getOMFactory();

            // Create the element
            final OMElement tpInfo = f.createOMElement((rootName == ElementName.FROM ? Q_FROM_PARTY : Q_TO_PARTY), piElement);
         ...
         *                              read completely
         */
        public static org.holodeckb2b.ebms3.persistency.entities.TradingPartner readElement(final OMElement tpElement) throws PackagingException {
            if (tpElement == null)
                return null; // If there is no element content, there is no data

            // Create the entity object
            final org.holodeckb2b.ebms3.persistency.entities.TradingPartner tpData = new org.holodeckb2b.ebms3.persistency.entities.TradingPartner();

            // Check for a Role element and use its value
            final OMElement roleElement = tpElement.getFirstChildWithName(Q_ROLE);
            if (roleElement != null) {
                final String role = roleElement.getText();
            ...
            while (it.hasNext()) {
                ...
                final String pid = pidElem.getText();
                final String pidType = pidElem.getAttributeValue(new QName(LN_PARTYID_TYPE));
                // The PartyId must have a value
                if (pid == null || pid.isEmpty())
                    throw new PackagingException("PartyId is required but found empty");

                final PartyId pidData = new PartyId();
                pidData.setId(pid);
     ...
     */
    public static void readElement(final OMElement piElement, final org.holodeckb2b.ebms3.persistency.entities.UserMessage umData) throws PackagingException {
        if (piElement == null)
            return;
