        ...
        if (Utils.isNullOrEmpty(waitingForRcpt)) 
            ...
            for (final IUserMessageEntity um : waitingForRcpt) {
                try {
                    ...
                    if (leg == null) {
                    	log.error("P-Mode [{}] of User Message [msgId={}] is not available!", um.getPModeId(), 
                    				um.getMessageId());
                    	// Set state to SUSPENDED as the P-Mode may only be temporarily unavailable and processing can
                    	// be resumed later 
                    	storageManager.setProcessingState(um, ProcessingState.SUSPENDED, "P-Mode not available");
                    ...
                    if (intervals != null && intervals.length > 0) {
                        ...
                        log.warn("No retry configuration defined in P-Mode [{}] of User Message [msgId={}]",
                        		 um.getPModeId(), um.getMessageId());
