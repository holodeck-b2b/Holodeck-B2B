...
public class CheckSignatureCompleteness extends AbstractUserMessageHandler {
    ...
    protected InvocationResponse doProcessing(MessageContext mc, EntityProxy<UserMessage> umProxy) throws Exception {
        ...
        Collection<OMElement> references = SecurityUtils.getSignatureReferences(mc);        
    ...
    private String getSOAPBodyIdRef(final MessageContext mc) {
        ...
        for(Iterator<OMAttribute> attributes = body.getAllAttributes(); attributes.hasNext() && bodyId == null;) {
     ...
     */
    private void createValueInconsistentError(MessageContext mc, final String msgId, final String plRef) {
        ValueInconsistent   error = new ValueInconsistent();        
