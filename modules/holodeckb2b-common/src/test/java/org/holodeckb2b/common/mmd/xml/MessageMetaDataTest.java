...
public class MessageMetaDataTest {
    ...
    public void test_CreateFromFile() throws Exception {
        ...
        try {
            ...
            final Iterator<IPartyId> it = mmd.getReceiver().getPartyIds().iterator();
            pid = it.next();
            ...
            final Iterator<IPayload> pls = mmd.getPayloads().iterator();
    ...
    public void test_CreateFromObject() {
        ...
        final CollaborationInfo ci = new CollaborationInfo();
        ci.setAction(T_UM1_ACTION);
        ci.setConversationId(T_UM1_CONVID);
        final org.holodeckb2b.common.messagemodel.Service svc = new org.holodeckb2b.common.messagemodel.Service(T_UM1_SERVICE);
        ci.setService(svc);
        final org.holodeckb2b.common.messagemodel.AgreementReference agreeRef = new org.holodeckb2b.common.messagemodel.AgreementReference();
        agreeRef.setPModeId(T_UM1_PMODEID);
        ci.setAgreement(agreeRef);
        um.setCollaborationInfo(ci);

        um.addMessageProperty(new org.holodeckb2b.common.messagemodel.Property(T_UM1_MSGPROP1_NAME, T_UM1_MSGPROP1_VALUE, T_UM1_MSGPROP1_TYPE));
        um.addMessageProperty(new org.holodeckb2b.common.messagemodel.Property(T_UM1_MSGPROP2_NAME, T_UM1_MSGPROP2_VALUE));

        Payload pl = new Payload();
        pl.setContainment(T_UM1_PAYLD1_CONTAINMENT);
        pl.setContentLocation(T_UM1_PAYLD1_LOC);
        pl.addProperty(new org.holodeckb2b.common.messagemodel.Property(T_UM1_PAYLD1_PROP_NAME, T_UM1_PAYLD1_PROP_VALUE));
        org.holodeckb2b.common.messagemodel.SchemaReference schemaRef =
                new org.holodeckb2b.common.messagemodel.SchemaReference(T_UM1_PAYLD1_SCHEMA_NS, T_UM1_PAYLD1_SCHEMA_LOC);
        schemaRef.setVersion(T_UM1_PAYLD1_SCHEMA_VER);
        pl.setSchemaReference(schemaRef);
        um.addPayload(pl);
        ...
        final MessageMetaData mmd = new MessageMetaData(um);
     ...
     * @param um2
     */
    private void assertEqualUM(final IUserMessage um1, final IUserMessage um2) {
        ...
        final ITradingPartner sender1 = um1.getSender();
        final ITradingPartner sender2 = um2.getSender();
        if (sender1 != null) {
            assertNotNull(sender2);
            assertEquals(sender1.getRole(), sender2.getRole());

            final Collection<IPartyId> pids1 = sender1.getPartyIds();
            final Collection<IPartyId> pids2 = sender2.getPartyIds();

            if(pids1 != null && pids1.size() > 0) {
                assertNotNull(pids2);
                assertEquals(pids1.size(), pids2.size());

                for(final IPartyId pid : pids1) {
                    boolean b = false;
                    for(final IPartyId pid2 : pids2) {
        ...
        final ITradingPartner receiver1 = um1.getSender();
        final ITradingPartner receiver2 = um2.getSender();
        if (receiver1 != null) {
            assertNotNull(receiver2);
            assertEquals(receiver1.getRole(), receiver2.getRole());

            final Collection<IPartyId> pids1 = receiver1.getPartyIds();
            final Collection<IPartyId> pids2 = receiver2.getPartyIds();
        ...
        final ICollaborationInfo ci1 = um1.getCollaborationInfo();
        final ICollaborationInfo ci2 = um2.getCollaborationInfo();
        ...
        if (ci1 != null) {
            ...
            final IService svc1 = ci1.getService();
            final IService svc2 = ci2.getService();
            if (svc1 != null) {
                assertNotNull(svc2);
            ...
            final IAgreementReference ar1 = ci1.getAgreement();
            final IAgreementReference ar2 = ci2.getAgreement();
            if (ar1 != null) {
                assertNotNull(ar2);
        ...
        if(props1 != null && props1.size() > 0) {
            ...
            for(final IProperty p1 : props1) {
                boolean b = false;
                for(final IProperty p2 : props2) {
        ...
        final Collection<IPayload> pl1 = um1.getPayloads();
        final Collection<IPayload> pl2 = um2.getPayloads();

        if(pl1 != null && pl1.size() > 0) {
            assertNotNull(pl2);
            assertEquals(pl1.size(), pl2.size());

            for(final IPayload p1 : pl1) {
                boolean b = false;
                for(final IPayload p2 : pl2) {
                    boolean m = false;
                    if (p1.getContentLocation() != null && p1.getContentLocation().equals(p2.getContentLocation())) {
                        ...
                        final ISchemaReference schema1 = p1.getSchemaReference();
                        final ISchemaReference schema2 = p2.getSchemaReference();
                        ...
                        if(props1 != null && props1.size() > 0 && props2 != null && props2.size() > 0) {
                            for(final IProperty prop1 : props1) {
                                final boolean mp = false;
                                for(final IProperty prop2 : props2) {
