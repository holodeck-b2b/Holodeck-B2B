...
public class HB2BMonitoringApp {
	...
	/**
	 * Redelivers or triggers redelivery of the message with the given CoreId.
	 *
	 * @param coreId	CoreId of the message unit to redeliver
	 * @since 9.0.0
	 */
	public void redeliverMessage(final String coreId) {
		try {
			coreAPI.redeliver(coreId);
			// Refresh the message status, we can use the MessageId of the first message unit because all message units
			// have the same MessageId
			msgUnitStatus.setMessageUnits(coreAPI.getMessageUnitInfo(msgUnitStatus.getMessageUnit(0).getMessageId()));
		} catch (RemoteException e) {
			e.printStackTrace(System.err);
			JOptionPane.showMessageDialog(mainWindow,
										  "An error occurred while redelivering the message:\n" + e.getMessage(),
										  "Error", JOptionPane.ERROR_MESSAGE);
		}
	}

	/**
	 * Resends the User Message with the given CoreId.
	 *
	 * @param coreId	CoreId of the User Message to resend
	 * @since 9.0.0
	 */
	public void resendUserMessage(final String coreId) {
		try {
			coreAPI.resend(coreId);
			// Refresh the message status, we can use the MessageId of the first message unit because all message units
			// have the same MessageId
			msgUnitStatus.setMessageUnits(coreAPI.getMessageUnitInfo(msgUnitStatus.getMessageUnit(0).getMessageId()));
		} catch (RemoteException e) {
	...
	/**
	 * Resumes processing of the suspended User Message with the given CoreId.
	 *
	 * @param coreId	CoreId of the suspended User Message
	 * @since 9.0.0
	 */
	public void resumeProcessing(final String coreId) {
		try {
			coreAPI.resumeProcessing(coreId);
			// Refresh the message status, we can use the MessageId of the first message unit because all message units
			// have the same MessageId
			msgUnitStatus.setMessageUnits(coreAPI.getMessageUnitInfo(msgUnitStatus.getMessageUnit(0).getMessageId()));
		} catch (RemoteException e) {
			e.printStackTrace(System.err);
			JOptionPane.showMessageDialog(mainWindow,
										  "An error occurred while resuming processing of the message:\n"
										  + e.getMessage(),
										  "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
