 ...
 * offer so the Core can store the meta-data on processed message units. The DAO structure follows the message model
 * with a generic DAO for operations that apply to all message unit types and specific DAOs for each message unit type.
 ...
 * <p>Note that the data access objects don't offer a generic update method that allows to save the complete state of an
 * entity object but only supply methods to change specific fields. This is done to simplify and allow optimizations
 * in the persistency provider implementation.
