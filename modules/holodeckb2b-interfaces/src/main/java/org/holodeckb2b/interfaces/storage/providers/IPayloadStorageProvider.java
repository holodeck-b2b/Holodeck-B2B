...
public interface IPayloadStorageProvider {
	...
	/**
	 * Creates a new {@link IPayloadContent} object that can be used to store the payload contents. The P-Mode may 
	 * specify or provide specific parameters hw the payload content must be stored, for example encrypted. As the 
	 * payloads of received and sent messages may need to be processed differently, also the direction of the User
	 * Message in which the payload is contained must be specified.
	 * 
	 * @param payloadId	the <i>PayloadId</i> of the payload which content are to be stored
	 * @param pmode		the P-Mode that governs the processing of the User Message the payload is contained in
	 * @param direction	the direction of the User Message in which the payload is contained
	 ...
	 */
	IPayloadContent createNewPayloadStorage(final String payloadId, final IPMode pmode, final Direction direction) 
																								throws StorageException;
	
	/**
	 * Retrieves the stored content of a payload.
	 * 
	 * @param payloadId	the <i>PayloadId</i> of the payload which content should be retrieved	
	 * @return {@link IPayloadContent} object to use for retrieving the payload content, or<br/>
	 * 		   <code>null</code> if there is no content for the given <i>PayloadId</i> 
	 * @throws StorageException when an error occurs retrieving the payload content
	 */
	IPayloadContent getPayloadContent(final String payloadId) throws StorageException;
	
	/**
	 * Removes the stored content of a payload.
	 * 
	 * @param payloadId	the <i>PayloadId</i> of the payload which content should be removed
	 * @throws StorageException	when an error occurs that prevents the <i>Payload Storage Provider</i> to remove
	 * 							the payload content. 
	 */
	void removePayloadContent(final String payloadId) throws StorageException;
}
