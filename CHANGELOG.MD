...
###### 2022-01-19
### Changed
* Update of log4j2 dependency to version 2.17.1 
* Logging of purge process. Change log level of individual removal message to DEBUG and added a summary message with level INFO on completion.  

## 2.1.9
###### 2021-12-15
### Fixed
* Vulnerability in log4j2 [CVE-2021-45046](https://cve.mitre.org/cgi-bin/cvename.cgi?name=CVE-2021-45046)
...
###### 2021-11-22
### Changed
* Updated version of dependencies to support Java 11 
...
* Fall back to default destination URL if no specific one has been specified for async Receipt or Error
* Pull worker stops pulling when the database connection is lost during execution
* Possible database connection leaks 

### Removed
* Examples from the distribution package

## 2.1.6
###### 2019-10-01
### Added
* Additional parameter to the "clean up" worker `org.holodeckb2b.ebms3.workers.PurgeOldMessagesWorker` to specify 
  period after which _Pull Requests_ and "Empty MPC" _Error_ message units should be removed.

### Fixed
* NPE when the worker reading the pull configuration is started before the Core has initialised the pull worker pool.
